...
$sql = "SELECT * FROM clientes WHERE correo = ? AND eliminado = 0"; // Solo usuarios no eliminados
...
if ($stmt = $con->prepare($sql)) {
    ...
    if ($res->num_rows > 0) {
        ...
        if ($pass == $empleado['pass']) {
            // Establecemos la sesión para el nombre y correo del usuario
            $_SESSION['nombre'] = $empleado['nombre']; // Guarda el nombre del empleado en la sesión
            $_SESSION['correo'] = $empleado['correo']; // Guarda el correo del empleado en la sesión
